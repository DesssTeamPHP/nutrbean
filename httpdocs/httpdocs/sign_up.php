<?php include('admin/core/configuration.php');
                                ...
                                {
								$select_cust                 =        "select * from user_details where id = ".$_GET["uid"]."";
								$cust_execute                =         mysql_query($select_cust); 
								$cust_results                =         mysql_fetch_assoc($cust_execute);	
								
								$fname    		             =         $cust_results['first_name'];
								$lname			             =         $cust_results['last_name'];
								$uaddress1		             =         $cust_results['address'];
								$ucity 		              	 =         $cust_results['city'];
								$ustate			             =         $cust_results['state'];
								$ucountry 		             =         $cust_results['country'];
								$phonenum		             =         $cust_results['prim_tele_phone'];
								$mailid 		             =         $cust_results['e_mail'];
								//$cmailid		             =         $cust2[e_mail];
								$user_id 		             =         $cust_results['user_id'];
								$password1 		             =         $cust_results['password'];
								$cpassword 		             =         $cust_results['password'];
								$active			             =         $cust_results['active'];
								$created_date	             =         $cust_results['created_date'];
								$uzip			             =         $cust_results['zip'];
								$type			             =         $cust_results['type'];
								...
								$uaddress                    =         $uaddress1;
								$select_user_detail          =         "SELECT * FROM user_details where e_mail='".$mailid."'";
								$selectuser_execute          =         mysql_query($select_user_detail);
								$user_num_Records            =         mysql_num_rows($selectuser_execute);
		
		                        if($user_num_Records==0)
		                        {
			                    $qryinsertuser               =         "INSERT INTO user_details values       ('','','$fname','$lname','$uaddress','$ucity','$ustate','$ucountry','$phonenum','$mailid','$mailid','$password1','1',now(),now(),'$uzip','2')";
    ...
    <article>
      ...
      <form action="" method="post" onSubmit="return validsignup();">
        ...
        <ul class="sign_in">
          ...
          <li>
            <input class="signin_text" type="text" value="<?=$mailid?>" name="mailid" id="mailid">
          </li>
          <li>
            <p>Password :</p>
          </li>
          ...
          <li>
            <textarea class="signin_text1" id="uaddress1" name="uaddress1" rows="" cols=""><?=$uaddress1?>
</textarea>
          </li>
          <li>
            <p>Country :</p>
