...
$get_order_data_final	 =	"SELECT * FROM order_tbl where  order_id = '".$_REQUEST['id']."' ";		
  ...
  <div class="main_content">
    <div class="row">
      <div class="col-sm-8 col-md-8">
        <h1 class="invoice_heading">Order <?php echo $Order_results["order_id"];?></h1>
      </div>
      <div class="col-sm-4 col-md-4">
        <p class="sepH_a"><span class="sepV_b text-muted">Order No.</span><strong><?php echo $Order_results["order_id"];?></strong></p>
        <p class="sepH_a"><span class="sepV_b text-muted">Order Date</span><strong><?php echo date('m/d/Y',strtotime($Order_results["order_created"]));?></strong></p>
    ...
    <div class="row">
      ...
      <div class="col-sm-4 col-md-4">
        <p><strong class="text-muted">Billing Address</strong></p>
        <address>
        <strong><?php 
		$qrychk_order	 	  = "select * from order_billing where order_id='$order_id'";
        $qrychk_qrychk_order  = mysql_query($qrychk_order);
        $nt111		 		  = mysql_fetch_assoc($qrychk_qrychk_order);
        $qrychk12	  = "select * from us_states where state_id='$nt111[state]'";
        $qrychkexe112 = mysql_query($qrychk12);
        $nt112		  = mysql_fetch_assoc($qrychkexe112);
        $state		  = $nt112['state_name'];	
		
		echo $nt111["first_name"].'&nbsp;'.$nt111["last_name"];?></strong><br>
        <?php echo $nt111['address'];?><br>
        <?php echo $nt111['city'];?><br>
        <?php echo $state;?><br>
        <?php echo $nt111['country'];?> &nbsp; <?php echo $nt111['zipcode'];?><br>
        </address>
      </div>
      <div class="col-sm-4 col-md-4">
        <p><strong class="text-muted">Shipping Address</strong></p>
        <?php  $qrychk11	  = "select * from order_shipping where order_id='$order_id'";
		...
		?>
        <address>
        <strong><?php echo $nt111["shipping_firstname"].'&nbsp;'.$nt111["shipping_lastname"];?></strong><br>
        <?php echo $nt111['shipping_address'];?><br>
        <?php echo $nt111['shipping_city'];?><br>
        <?php echo $state;?><br>
        <?php echo $nt111['shipping_country'];?> &nbsp; <?php echo $nt111['shipping_zip'];?><br>
        </address>
      </div>
    </div>
    <div class="row">
      <div class="col-sm-12 col-md-12">
        <table class="table">
          ...
          <tbody>
        <?php      $qrychk1="select * from order_details where order_id='$order_id'";
...
echo 	'<tr>
              ...
              <td>'.$nt11['product_quantity'].'</td>
              <td class="invoice_tar">$'.number_format($total,2,'.','').'</td>
            </tr>';
          ...
          $i++;
           ...
           ?> 
            <tr>
              <td colspan="3">&nbsp;</td>
              <td class="invoice_tar"><p class="sepH_a"><span class="sepV_b text-muted">Subtotal</span>$<?php echo number_format($gtotal,2,'.','');?></p>
                <p class="sepH_a"><span class="sepV_b text-muted">Shipping Amount</span>$<?php if(trim($Order_results['shipping_amt'])!="") echo  $Order_results['shipping_amt']; else echo '0.00';?></p>
                <p class="sepH_a"><span class="sepV_b text-muted">Tax </span>$ <?php if(trim($Order_results['tax_amt'])!="")  echo $Order_results['tax_amt']; else echo '0.00';?></p>
                <p class="sepH_a text-success"><strong><span class="sepV_b">Total</span>$<?php if(trim($Order_results['billing_amt'])!="") echo number_format($Order_results['billing_amt'],2,'.',''); else echo '0.00';?></strong></p></td>
            </tr>
          </tbody>
        </table>
      </div>
