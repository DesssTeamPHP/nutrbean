	...
	gebo_pass_check = {
		init: function() {
			$("#pass_check").complexify({
				...
				if (!valid) {
					$('#pass_progress .progress-bar').css({'width':complexity + '%'}).removeClass('progress-bar-success').addClass('progress-bar-danger');
				} else {
					$('#pass_progress .progress-bar').css({'width':complexity + '%'}).removeClass('progress-bar-danger').addClass('progress-bar-success');
				}
			});
		}
	};
