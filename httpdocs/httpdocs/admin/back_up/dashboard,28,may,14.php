  ...
  <div class="main_content">
    <div class="row">
      <div class="col-sm-12 tac">
        <ul class="ov_boxes">
          <?php 
	$start_date_ga = date('Y-m-d',strtotime('today - 7 days'));
	$end_date_ga = date('Y-m-d');		
	 	 $get_order_data ="SELECT DATE(oredered_date) AS date, SUM(total_amount) AS billing_amount FROM ".ORDERDETAILS." where  oredered_date >= '".$start_date_ga."' and oredered_date <='".$end_date_ga."'  GROUP BY oredered_date DESC limit 0,7";		
	...
	$start_date_ga_month = date('Y-m-d',strtotime('today - 30 days'));
	$end_date_ga_month = date('Y-m-d');	
		
	
		 $get_order_data_month ="SELECT DATE(oredered_date) AS date, SUM(total_amount) AS billing_amount FROM ".ORDERDETAILS." where  oredered_date >= '".$start_date_ga_month."' and oredered_date <='".$end_date_ga_month."'  GROUP BY oredered_date DESC limit 0,30";
		...
		{
		$total_cost_month  = $total_cost_monthk + $row_fetch_month['billing_amount'];
    ...
    <div class="row">
      <div class="col-sm-6 col-lg-6">
        ...
        <table class="table table-striped table-bordered mediaTable">
          ...
          <tbody>
          <?php   $get_order_latest ="SELECT first_name,order_id,DATE(oredered_date) AS date, SUM(total_amount) AS billing_amount FROM ".ORDERDETAILS." GROUP BY order_id  ORDER BY oredered_date DESC limit 0,7";		
		  ...
		  <tr>
              ...
              <td><?php echo $fetch_order_details_latest['first_name'];?></td>
